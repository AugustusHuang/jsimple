...
(defparameter +decimal-digits+ "[0-9]+")
...
(defparameter +line-continuation+ "\\\\(\r\n|\r|\n)")
(defparameter +octal-escape-sequence+ "(?:[1-7][0-7]{0,2}|[0-7]{2,3})")
(defparameter +hex-escape-sequence+
  (concatenate 'string "[x]" +hex-digit+ "{2}"))
(defparameter +unicode-escape-sequence+
  (concatenate 'string "[u]" +hex-digit+ "{4}"))
(defparameter +single-escape-character+ "[\'\"\\\\bfnrtv]")
(defparameter +non-escape-character+ "[^\'\"\\\\bfnrtv0-9xu]")
(defparameter +character-escape-sequence+
  (concatenate 'string +single-escape-character+ "|" +non-escape-character+))
(defparameter +escape-sequence+
  (concatenate 'string +character-escape-sequence+ "|" +octal-escape-sequence+
	       "|" +hex-escape-sequence+ "|" +unicode-escape-sequence+))
(defparameter +double-string-character+
  (concatenate 'string "([^\"\\\\\n\r]+)|(\\\\" +escape-sequence+ ")|"
	       +line-continuation+))
(defparameter +single-string-character+
  (concatenate 'string "([^\'\\\\\n\r]+)|(\\\\" +escape-sequence+ ")|"
	       +line-continuation+))
(defparameter +string-literal+
  (concatenate 'string "(\"" +double-string-character+ "*\")|(\'"
	       +single-string-character+ "*\')"))
(defparameter +regular-expression-non-terminator "[^\n\r]")
(defparameter +regular-expression-backslash-sequence+
  (concatenate 'string "\\\\" +regular-expression-non-terminator+))
(defparameter +regular-expression-class-char+
  (concatenate 'string "[^\n\r\]\\\\]|" +regular-expression-backslash-sequence+))
;; NOTE: "\[" will be handled as [, which is a invalid regular expression.
(defparameter +regular-expression-class+
  (concatenate 'string "\\[" +regular-expression-class-char "*\\]"))
(defparameter +regular-expression-flags+
  (concatenate 'string +identifier-part+ "*"))
(defparameter +regular-expression-first-char+
  (concatenate 'string "([^\n\r\*\\\\\/\\[])|"
	       +regular-expression-backslash-sequence+ "|"
	       +regular-expression-class+))
(defparameter +regular-expression-char+
  (concatenate 'string "([^\n\r\\\\\/\\[])|"
	       +regular-expression-backslash-sequence+ "|"
	       +regular-expression-class+))
(defparameter +regular-expression-body+
  (concatenate 'string +regular-expression-first-char+
	       +regular-expression-char+ "*"))
(defparameter +regular-expression-literal+
...
(deflexer jsimple-lexer
  ...
  ("[0-9]+" (return (values 'numeric (int %0))))
  ("[:alpha:][:alnum:]*" (return (values 'identifier %0)))
  ("[:space:]+")
  ())
